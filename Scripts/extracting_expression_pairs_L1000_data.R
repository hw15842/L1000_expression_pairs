...
################################################
### extracting_expression_pairs_L1000_data.R ###
################################################
...
load("gene_expression_pairs.rdata")
...
gene_expression_pairs_l1000 <- subset(gene_expression_pairs, Var1 %in% d2$pert_iname) 
...
gene_expression_pairs_l1000 <- subset(gene_expression_pairs_l1000, Var2 %in% sort_entrez$hgnc_symbol)  
...
datalist=list()
# pull out Z scores
for (i in 1:nrow(gene_expression_pairs_l1000)) {
  cols <- which(d2$pert_iname==as.character(gene_expression_pairs_l1000[i,1])) # this is the row of the matrix and the first protein
  row <-which(sort_entrez$hgnc_symbol==as.character(gene_expression_pairs_l1000[i,2])) # this is the column of the matrix and the second protein
  indices <- cbind(row,cols)
  #   if (nrow(d2[cols,])==1) {
  #     paste(prot_pairs_l1000[i,],d2[cols,],ds@mat[indices])  
  #   } else {
  datalist[[i]] <- cbind(gene_expression_pairs_l1000[i,],d2[cols,][,1],ds@mat[indices])  # cbind the row of prot_pairs_l1000, the sig_id and the corresponding z score of the col and row of the matrix
}
